...
import { getClassicsProducts, sortClassicsProductsByPrice, sortClassicsProductsByPriceDesc,sortClassicsProductsByDefault } from "../../service";
...
export default function Classics() {
  ...
  const handleSort = (event) => {
    if(event.target.value === "default")
      return sortClassicsProductsByDefault().then((response) => {
        setClassicsProducts(response); 
      });   
    if(event.target.value === "price-ascending")
      return sortClassicsProductsByPrice().then((response) => {
        setClassicsProducts(response); 
      });
    if(event.target.value === "price-descending")
      return sortClassicsProductsByPriceDesc().then((response) => {
        setClassicsProducts(response); 
      });
  };
